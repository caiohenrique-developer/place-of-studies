...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if ((0, _dateFns.isBefore)(appointmentDate, Date.now())) {
      throw new _AppError.default("You can't to create an appointment on a past date.");
    }

    if (user_id === provider_id) {
      throw new _AppError.default("You can't to create an appointmente with yourself.");
    }

    if ((0, _dateFns.getHours)(appointmentDate) < 8 || (0, _dateFns.getHours)(appointmentDate) > 17) {
      throw new _AppError.default('You can only create appointments between 8am and on the max 17pm.');
    }
    ...
    const formattedDate = (0, _dateFns.format)(appointmentDate, "MMM/dd/yyyy - EEE 'ás' kk:mm:ss");
