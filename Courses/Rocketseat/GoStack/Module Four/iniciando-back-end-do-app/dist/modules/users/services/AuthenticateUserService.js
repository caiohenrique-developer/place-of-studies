...
let AuthenticateUserService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const user = await this.usersRepository.findByEmail(email); // if user is not exist, return error
    ...
    const passwordMatched = await this.hashProvider.compareHash(password, user.password); // if password incorrect return error
